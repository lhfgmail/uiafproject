...
HINSTANCE	g_hGetWordInst = NULL;	//nhw32.dll的句柄
BOOL	g_bInGetWord = FALSE;	//是否进入取词状态
char	g_TextBuffer[NHD_MAX_TEXTLEN];	//取到的词
RECT	g_WordRect;
UINT	g_WM_GetWordOk;				//注册一个windows消息 "BL_HASSTRING"
UINT	g_nGWTimerID = 0;
HWND	g_hNHMainWin = NULL;	//Dome.exe程序的窗口句柄
HWND	g_hFlyWin = NULL;			//取词辅助窗口，该窗口会在鼠标位置处显示，并快速消失
...
//-----------------------------------------------------------------------------
// load nhw32.dll 装载取词DLL,同时取出取词DLL中的两个函数fpBL_GetText32，fpBL_SetFlag32
// 最后执行SetNHW32，笔者解释说 验证NT 环境???
//-----------------------------------------------------------------------------
...
BOOL NHD_LoadGetWordLib(void)
{
	...
	if(SetNHW32)
	{
		if(!SetNHW32())	//安装Hook, SetWindowsHookEx(WH_GETMESSAGE, GetMsgProc, g_hinstDll, 0);
		{
...
//-----------------------------------------------------------------------------
// begin get words 取词入口
//传入当前鼠标的位置
//-----------------------------------------------------------------------------
void NHD_BeginGetWord(POINT& ptMousePos)
{
	char szAppClassName[NHD_CLASSNAME_LEN + 1];	//目标窗口类名
	HWND hAppWin;	//目标窗口句柄
    int  nFlyWinLeft;
	int  nFlyWinWidth;
	RECT rcAppWin;	//目标窗口矩形范围
	
	//get window from mouse point; 获取鼠标位置下的窗口句柄.如果该点在静态文本控制之上，返回值是在该静态文本控制的下面的窗口。
	...
	//启动计时器去取缓存中放的文本，用Timer可能是防止界面卡死
